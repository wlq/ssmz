...
public class AuthServiceImpl extends BaseServiceImpl implements AuthServiceI {
	...
	public List<Auth> treegrid(Auth auth) {
		...
		if (auth != null && auth.getId() != null) {
			tauths = authDao.find("from Tauth t where t.tauth.cid = ? order by t.cseq", new Object[] { auth.getId() });
		} else {
			tauths = authDao.find("from Tauth t where t.tauth is null order by t.cseq");
		}
	...
	private void del(String cid) {
		...
		if (t != null) {
			authDao.executeHql("delete Troletauth t where t.tauth = ?", new Object[] { t });
