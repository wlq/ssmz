...
<script type="text/javascript" charset="utf-8">
	var datagrid;
	var researchAddDialog;
	var researchAddForm;
	var researchEditDialog;
	var researchEditForm;
	var cdescEdit;
	var cdescAdd;
	var showCdescDialog;
	$(function() {
		datagrid = $('#datagrid').datagrid({
			...
			url : 'researchAction!datagrid.action',
			title : '现状调查报告列表',
			iconCls : 'icon-save',
			pagination : true,
			pagePosition : 'bottom',
		...
		researchAddForm = $('#researchAddForm').form({
			url : 'researchAction!add.action',
			success : function(data) {
				...
				if (json && json.success) {
					...
					researchAddDialog.dialog('close');
				} else {
					$.messager.show({
						title : '失败',
						msg : json.msg
		...
		researchAddDialog = $('#researchAddDialog').show().dialog({
			title : '添加现状调查报告信息',
			modal : true,
			closed : true,
			maximizable : true,
			buttons : [ {
				text : '添加',
				handler : function() {
					researchAddForm.submit();
				}
			}]
		});

		researchEditForm = $('#researchEditForm').form({
			url : 'researchAction!edit.action',
			success : function(data) {
				var json = $.parseJSON(data);
				if (json && json.success) {
					$.messager.show({
					...
					datagrid.datagrid('reload');
					researchEditDialog.dialog('close');
				} else {
					$.messager.show({
						title : '失败',
						msg : json.msg
		...
		researchEditDialog = $('#researchEditDialog').show().dialog({
			title : '编辑现状调查报告信息',
			modal : true,
			closed : true,
			maximizable : true,
		...
		cdescAdd = $('#cdescAdd').xheditor({
			...
			html5Upload : true,
			upMultiple : 4,
			upLinkUrl : 'researchAction!upload.action',
			upLinkExt : 'zip,rar,txt,doc,docx,xls,xlsx',
			upImgUrl : 'researchAction!upload.action',
	...
	function add() {
		researchAddForm.find('input,textarea').val('');
		$('div.validatebox-tip').remove();
		researchAddDialog.dialog('open');
	}
	function del() {
		...
		if (rows.length > 0) {
			$.messager.confirm('请确认', '您要删除当前所选项目？', function(r) {
				if (r) {
					...
					$.ajax({
						url : 'researchAction!delete.action',
						data : {
							ids : ids.join(',')
						},
						dataType : 'json',
	...
	function edit() {
		...
		if (rows.length == 1) {
			...
			$.ajax({
				url : 'researchAction!showDesc.action',
				data : {
					cid : rows[0].cid
				},
				dataType : 'json',
				cache : false,
				success : function(response) {
					researchEditForm.form('load', response);
					$('div.validatebox-tip').remove();
					researchEditDialog.dialog('open');
		...
		if (node) {
			$.messager.confirm('询问', '您确定要更改审核的状态？',
					function(b) {
						if (b) {
							$.ajax({
								url : 'researchAction!changeFlag.action',
								data : {
									cid : node.cid,
									cstatus : node.cflag
								},
...
<body class="easyui-layout">
	...
	<div id="researchAddDialog" style="display: none;width: 600px;height: 400px;" align="center">
		<form id="researchAddForm" method="post">
			<table class="tableForm">
				...
				<tr>
					<th>现状调查质量说明</th>
					<td><input name="cnote" class="easyui-validatebox" required="true"   missingMessage="请填写现状调查质量说明" /></td>				
					<th>现状调查数据存储状态</th>
					<td>
				...
				<tr>
					...
					<th>现状调查单位</th>
					<td><input name="cunit" class="easyui-validatebox" required="true"   missingMessage="请填写现状调查单位" /></td>
				</tr>
				<tr>
					<th>联系方式</th>
	...
	<div id="researchEditDialog" style="display: none;width: 600px;height: 400px;" align="center">
		<form id="researchEditForm" method="post">
