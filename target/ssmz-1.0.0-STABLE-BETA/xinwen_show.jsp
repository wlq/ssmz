...
i = (i-1)*7;
 ...
 String sql_1="select COUNT(*)  from Txinwen";
 ...
 String sql= "select cid,cname,ccreatedatetime,cabstract from Txinwen order by CCREATEDATETIME desc limit "+i.intValue()+",7";
	...
	<tr>
		<td>
                        <div class="liebiaomiddleyiceng1" id="only">
                            <h3>
                                <a href='xinwendetail.jsp?id=<%=rs.getString(1)%>' target="_blank"
                                    style=" white-space: nowrap; text-overflow: ellipsis; overflow: hidden; width:550px;">
                                <%=rs.getString(2)%></a>

                                <span>发表于:<%=rs.getString(3).substring(0,10)%></span></h3>
                            <div class="liebiaomiddleyiceng2"  style=" overflow: hidden;">
                                <p class="p2"><%=rs.getString(4)%></p>
                            </div>
                            <span class="xiamianxiangqing"><a href="xinwendetail.jsp?id=<%= rs.getString(1) %>" target="_blank" >查看详情&gt;&gt;</a></span>
                        </div>
                    </td>
	</tr>
<% 
...
%> 	
                ...
                <div class="leibiao">
                   ...
                   <span id="lblPages">
                   <% if(pagenum != 1){%>
                   <a href='xinwen_show.jsp?page=1' class='pageS1'>&lt;&lt;</a>
                   <%}%>
                   
                   <%if((pagenum -1) > 0 ) 
                   {
                   %>
                   <a href='xinwen_show.jsp?page=<%=pagenum-1%>' class='pageS1'>&lt;</a>
                   <%}
                   %>
                   <%if((pagenum -3) > 0 ) 
                   {
                   %>
                   <a href='xinwen_show.jsp?page=<%=pagenum-3 %>' class='pageS1' ><%=pagenum-3 %></a>
                   <%}
                   %>
                    <%if((pagenum -2) > 0 ) 
                   {
                   %>
                   <a href='xinwen_show.jsp?page=<%=pagenum-2 %>' class='pageS1' ><%=pagenum-2 %></a>
                   <%}
                   %>
                    <%if((pagenum -1) > 0 ) 
                   {
                   %>
                   <a href='xinwen_show.jsp?page=<%=pagenum-1 %>' class='pageS1' ><%=pagenum-1%></a>
                   <%}
                   %>
                   ...
                   {
                   %>
                   <a href='xinwen_show.jsp?page=<%=maxnumber %>' class='pageS1'>&gt;&gt;</a>
                   <%}
                   %>
                       转到第 <input type='text' class='text' id='txtInput'/>页 
                       <input type='button'  value='跳转' class='pageS2' 
                       onclick=" var strUrl='xinwen_show.jsp?page=#pagenum#' ; 
